...
1. 표현 방식
  ...
  ||인접 행렬|인접 리스트|
|:-|:-|:-|
|표현방식|- 파이썬에선 이중 리스트로 표현<br>- 간선 상의 비용을 리스트로 나타낸다.<br>  - 자기 자신과의 비용: 0<br> - 이어지지 않은 경우: INF (INF = 9999999 같은 식으로 표현)|-리스트로 그래프의 연결 관계를 표현하는 방식<br>- 이중 리스트로 표현<br>- (노드, 간선)의 형식으로 정보 저장|
|효율|두 노드 간 연결 여부 검색|메모리, 한 노드에 연결된 모든 노드 검색|

2. 동작 방식
  - 탐색 시작 노드를 스택에 삽입, 방문 처리(스택에 한 번 삽입된 노드가 다시 삽입되지 않게 체크하는 것을 의미함)
  - 스택의 최상단 노드에 방문하지 않은 인접 노드가 있으면 그 인접 노드를 스택에 넣고 방문 처리를 한다. 방문하지 않은 인접 노드가 없으면 스택에서 최상단 노드를 꺼낸다.
    - 방문하지 않은 인접 노드가 여러 개일 경우 번호가 낮은 순서부터 처리하는 것이 일반적
    - 최상단 노드를 꺼낸 경우 그 바로 아래 있던 노드가 최상단 노드가 되는 것이고, 이 노드를 기준으로 인접한 노드가 있는지 여부를 확인하는 식으로 진행한다.
  - 위의 과정을 수행불가할 때까지 반복한다.
...
1. 동작 방식
  ...
  - 큐에서 노드를 꺼내 해당 노드의 인접 노드 중 방문하지 않은 노드를 모두 큐에 삽입하고 방문 처리를 함.
  - 위의 과정을 수행불가할 때까지 반복
  - 노드의 탐색 순서 = 큐에 들어간 순서
  
2. 구현
  - 큐 활용 = deque 라이브러리 사용 (from collections import deque)
  - O(N), 일반적으로 DFS보다 수행시간이 좋은 편
...
> ✔✔✔🤔
2차원 배열에서의 탐색 문제 = 그래프 형태로 바꿔서 생각하기
